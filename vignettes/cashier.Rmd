...
```{r}
postings <- postings |> mutate(across(ends_with("date"), as.character))
ledger$postings$data |> rows_append(postings |> slice_head(n = 10), copy = TRUE, in_place = TRUE)
ledger$postings$data
ledger$postings$data |> rows_append(postings |> slice_head(n = 4), copy = TRUE, in_place = TRUE)
ledger$postings$data
ledger$postings$data |> rows_append(postings, copy = TRUE, in_place = TRUE)
ledger$postings$data
```
